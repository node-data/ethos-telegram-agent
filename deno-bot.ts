...
function formatProfileMessage(
  ...
  message +=
    `👤 <b>User: <a href="${profileUrl}">${finalDisplayName}</a></b>\n\n`;
  ...
  message += `📊 <b>Reviews:</b>\n`;
  message += `\n`;
  message += `• Total Received: ${reviews.received} (${
    reviews.positiveReviewPercentage.toFixed(1)
  }%)\n`;
  ...
  message += `\n`;
  message +=
    `• <a href="${profileUrl}?modal=review">Review ${finalDisplayName}</a>\n`;
  message += `\n`;
  ...
  if (reviews.received > 0) {
    ...
    if (vouches.balance.deposited > 0) {
      message += `• Vouched for others: ${
        vouches.balance.deposited.toFixed(4)
      }e (${vouches.count.deposited})\n`;
    }
  }

  message += `\n`;
  message +=
    `• <a href="${profileUrl}?modal=vouch">Vouch for ${finalDisplayName}</a>\n`;
