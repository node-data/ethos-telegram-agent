    ...
    try {
      ...
      return new Response(
        JSON.stringify({
          ...
          message:
            `Test reminder sent to ${reminderResult.success}/${usersForHour.length} users scheduled for ${testHour}:00 UTC (${reminderResult.skipped} skipped - tasks completed)`,
...
<i>This was a test message. Use /disable_task_refresh to turn these off.</i>
          ...
          console.error(
            `Failed to send test task refresh notification to user ${chatId}:`,
            error,
          );
          failureCount++;
        }
      ...
      return new Response(
        JSON.stringify({
          ...
          failed: failureCount,
          message:
            `Test task refresh notification sent to ${successCount}/${taskRefreshUsers.length} opted-in users`,
          optInPercentage: Math.round(
            (taskRefreshUsers.length / Math.max(allUsers.length, 1)) * 100,
          ),
        }),
        {
          headers: { "Content-Type": "application/json" },
    ...
    try {
      ...
      if (!chatIdParam) {
        return new Response(
          JSON.stringify({
            success: false,
            error:
              "Missing chatId parameter. Use: /test-reminder-user?chatId=YOUR_CHAT_ID",
          }),
          {
            status: 400,
            headers: { "Content-Type": "application/json" },
      ...
      try {
        ...
        if (userkey) {
          ...
          const contributionStatus = await checkDailyContributionStatus(
            userkey,
          );
